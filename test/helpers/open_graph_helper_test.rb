...
class OpenGraphHelperTest < ActionView::TestCase
  ...
  test "text-only post for subscribed account should have custom open graph image" do
    with_og_env_vars do
      ...
      assert_equal "https://og.example.com/og?avatar=http%3A%2F%2Ftest.host%2Fapple-touch-icon.png&blogTitle=%40joel&title=The+Art+of+Street+Photography", open_graph_image
    end
  end
