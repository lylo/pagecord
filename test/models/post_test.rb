require "test_helper"

class PostTest < ActiveSupport::TestCase
  include ActiveJob::TestHelper

  test "should be valid with body" do
    @post = Post.new(user: users(:joel), body: "Test post", html: false)
    assert @post.valid?
  end

  test "should not be valid without body or title" do
    @post = Post.new(user: users(:joel), body: nil, title: nil, html: false)
    assert_not @post.valid?
  end

  test "published at should be set on create if not provided" do
    post = users(:joel).posts.create! title: "my new post", body: "this is my new post", html: false

    assert_equal post.created_at, post.published_at
  end

  test "published at should be set" do
    published_at = 1.day.ago
    post = users(:joel).posts.create! title: "my new post", body: "this is my new post", html: false, published_at: published_at

    assert_equal published_at.to_time.to_i, post.published_at.to_time.to_i
  end

  test "post with blank title and body should be invalid" do
    refute users(:joel).posts.build(title: "", body: "", html: false).valid?
  end

  test "should strip empty tags before save" do
    post = posts(:two)
    post.body = "<p>Test</p><p></p><div></div>"
    post.save!

    assert_equal "<p>Test</p>", post.body.body.to_s.strip
  end
end
