class App::FeedController < AppController
  ...
  def index
    ...
    @posts = Post.posts.where(blog_id: followed_blog_ids).visible.order(published_at: :desc)
  ...
  def private_rss
    followed_blog_ids = @user.followed_blogs.pluck(:followed_id)
    @posts = Post.posts.where(blog_id: followed_blog_ids).visible.order(published_at: :desc).limit(30)
