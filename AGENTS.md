...
- **Toggles are singular resources.** An on/off preference is `create`/`destroy` on a `resource`, reached with `button_to` — `App::Posts::DetailsController` is the pattern, and `routes.rb` has many more (`resource :restoration`, `resource :spotlight_exclusion`, `resource :avatar`). A preference with a value is `update` on a singular resource taking the value as a param — `App::Pages::SortOrdersController`.
